...
case ${APP_NAME} in
    'sh')
        erl -setcookie c847efcb974ad5164d5d867d1bb2f0f9 -name sh@127.0.0.1 -epmd_port 27100 -epmd "epmd -port 27100 -daemon" -remsh health_${PORT}@${IP};;
    'ps')
        show_info ;;
    '-h'|'h'|'help')
        show_help;;
    ...
    'scp_rar')
	    scp health_*.tar.gz root@120.78.159.16:/root/project/health/;;

    'scp')
	    scp _build/default/lib/$2/ebin/$3.beam root@120.78.159.16:/root/project/health/${VERSION}/_build/default/lib/$2/ebin/;;
    '+S')
        erlc +"'S'" -I frame/include/ frame/test/test_trie.erl;;
    'release')
        mysqldump -u root -p123456 --port 27199 --databases test_health > ${VERSION}.sql
