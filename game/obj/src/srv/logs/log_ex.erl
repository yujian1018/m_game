...
log_share(Uid, PlatformType, CssType, OpState) ->
    ...
    log_share ! erl_string:sql([(integer_to_binary(Uid)), CTimes, (integer_to_binary(PlatformType)), (integer_to_binary(CssType)), (integer_to_binary(OpState))]).
...
log_invite(Uid, PlatformType, CssType, OpState) ->
    CTimes = integer_to_binary(erl_time:now()),
    log_invite ! erl_string:sql([(integer_to_binary(Uid)), CTimes, (integer_to_binary(PlatformType)), (integer_to_binary(CssType)), (integer_to_binary(OpState))]).


log_task(Uid, ChainId, Index) ->
    Data = <<"insert into log_task (uid, chain_id, `index`, u_times) values ",
        (erl_string:sql([Uid, ChainId, Index, erl_time:now()]))/binary,
